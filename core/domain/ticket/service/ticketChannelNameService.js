...
const buildTicketChannelName = (ticketNumber, channelUuid) => `${OPEN_PREFIX}${ticketNumber}-${channelUuid}`.toLowerCase();
...
const normalizeOpenName = name => {
	const base = stripOpenPrefix(stripClosedPrefix(name));
	return base.startsWith(OPEN_PREFIX) ? base : `${OPEN_PREFIX}${base}`;
};
